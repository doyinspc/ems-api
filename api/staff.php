...
include '../connect/connectstaff.php';
$op = new Db;
...
if($_SERVER['REQUEST_METHOD'] === 'GET')
{
	...
	if($cat === 'roomsta')
	{
		...
		$data1 = $op->selectroomstatistics($dates, 1); //get all bookins for that day
		$data2 = $op->selectroom($query, 1);//get all rooms
		$data3 = $op->selectroomstatistics($dates, 1);//get all rooms
		$data =  [$data1, $data2, $data3];
	}
	if($cat === 'roomana')
	{
		
		$data1 = $op->selectroomstatistics($query, 2); //get all bookins for that day
		$data2 = $op->selectroomstatistics($query, 3);//get all rooms
		$data3 = $op->selectroomstatistics($query, 2);//get all rooms
		$data =  [$data1, $data2, $data3];
	}
	if($cat === 'mainana')
	{
...
if($_SERVER['REQUEST_METHOD'] === 'POST')
{
	...
	if($cat === 'login')
	{
		...
		$data = $op->selectOne($table, NULL, array('employment_no' => $username));
		$data = (array) $data;
		if($data && isset($data['id']) && $data['id'] > 0)
		{
			if($data['passwd'] === md5($password) && $table === 'user_types')
			{
				$response = array(
				"status" => "success",
            	"error" => false,
